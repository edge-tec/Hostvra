      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-surface-900 border border-surface-800 rounded-xl p-4">
          ...
          <div className="flex items-center gap-2 w-full sm:w-72 bg-slate-50 dark:bg-[#121824] border border-slate-300 dark:border-surface-700 rounded-xl px-3.5 py-2 shadow-xs focus-within:border-[#20a53a] focus-within:ring-2 focus-within:ring-[#20a53a]/20 transition-all">
            ...
            />
            {searchTerm && (
              <button onClick={() => setSearchTerm('')} className="text-slate-400 hover:text-slate-600 dark:hover:text-white p-0.5">
                ...
                {filteredRecords.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-[#151d2d] transition-colors">
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 rounded bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 font-bold">
                        {r.type}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-bold text-slate-950 dark:text-white">{r.name}</td>
                    <td className="px-6 py-4 text-slate-800 dark:text-slate-200 font-medium truncate max-w-xs">
                      {r.type === 'MX' && <span className="text-amber-600 dark:text-amber-400 mr-2">[Priority {r.priority}]</span>}
                      {r.content}
                    </td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-400 font-medium">{r.ttl}s</td>
                    <td className="px-6 py-4">
                      {r.proxied ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-semibold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 font-sans">
                          <Cloud className="w-3 h-3 fill-amber-500 text-amber-500" /> Proxied
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-semibold bg-slate-100 dark:bg-surface-800 text-slate-700 dark:text-slate-400 border border-slate-200 dark:border-surface-700 font-sans">
                          DNS Only
                        </span>
                      )}
                    </td>
